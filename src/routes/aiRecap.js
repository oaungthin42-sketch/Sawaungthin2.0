...
Watch the entire video from start to end. Produce a literal, chronological, shot-by-shot timeline of what is actually visible on screen — not the plot, just what you can directly observe: who/what is on screen, what action is happening, and where each shot starts and ends in seconds. Cover the ENTIRE video with no large unexplained gaps (no gap longer than roughly 2-3 seconds between consecutive timeline entries). The shot-by-shot log must be near-continuous from start to finish. Skipping large stretches of the video, especially the ending, is NOT acceptable. Each timeline entry's "description" should be a short, literal, factual description of what is visible (e.g. "A man in a black jacket argues with a woman at a doorway", "Wide shot of a car driving down a street at night") — do not interpret or narrate meaning yet, just describe what is shown.

STEP 2 — WRITE THE RECAP NARRATION USING ONLY THE TIMELINE FROM STEP 1:
Now write the Burmese-language movie recap narration (third-person, explanatory storytelling — see style rules above). For EVERY narration segment, its "source_start" and "source_end" MUST exactly match (or be a sub-range within) one of the spans you already wrote in your own STEP 1 timeline. Do not approximate or invent plausible-sounding timestamps afterward; they must be real sub-ranges of your STEP 1 timeline entries. Pick whichever timeline entry/entries actually show what that narration segment describes — if no timeline entry matches well, choose the closest one that still shows the same character(s) or setting, rather than an unrelated one.
