...
try:
    ...
    logging.info(f"Downloading faster-whisper 'small' model into {os.environ['HF_HOME']}...")
    model = None
    try:
        ...
        try:
            model = WhisperModel("small", device="cpu", compute_type="float32", cpu_threads=1, num_workers=1)
        except Exception as e2:
            logging.warning(f"Failed download with compute_type='float32' ({e2}). Retrying with auto...")
            model = WhisperModel("small", device="cpu", cpu_threads=1, num_workers=1)
