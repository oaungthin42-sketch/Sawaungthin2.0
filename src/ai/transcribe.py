...
def main():
    ...
    model = None
    try:
        model = WhisperModel("small", device="cpu", compute_type="int8", cpu_threads=4, num_workers=2)
    except Exception as e:
        sys.stderr.write(f"Warning: Failed to load model with compute_type='int8': {e}\n")
        try:
            model = WhisperModel("small", device="cpu", compute_type="float32", cpu_threads=4, num_workers=2)
        except Exception as e2:
            sys.stderr.write(f"Warning: Failed to load model with compute_type='float32': {e2}\n")
            try:
                model = WhisperModel("small", device="cpu", cpu_threads=4, num_workers=2)
