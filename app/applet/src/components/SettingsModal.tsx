...
import { Key, Eye, EyeOff, Save, XCircle, Trash, Mic, Loader2, Play, Settings2, Sliders, Settings } from 'lucide-react';
...
export function SettingsModal({
    ...
    const renderSelect = (id: string, label: string, options: {value: string, label: string}[], description?: string) => {
        const value = settings[id]?.value || options[0].value;
        return (
            <div key={id} className="p-4 rounded-xl bg-gray-950/50 border border-gray-800/50 mb-4">
                <label className="block text-sm font-medium text-gray-300 mb-1">{label}</label>
                {description && <p className="text-xs text-gray-500 mb-3">{description}</p>}
                <select
                    className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors"
                    value={value}
    ...
    return (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-xl mb-8 space-y-8">
            ...
            <section>
                <div className="flex items-center gap-2 mb-4 text-indigo-300 border-b border-gray-800 pb-2">
                    ...
                    <h3 className="text-lg font-medium">Voice & Audio Settings</h3>
                </div>
