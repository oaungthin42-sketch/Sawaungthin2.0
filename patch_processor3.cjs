...
const startIndex = content.indexOf('for (let i = 0; i < authTimeline.length; i++) {');
const endIndex = content.indexOf('if (current_time < state.audioDuration) {');
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    fs.writeFileSync('src/workers/processor.js', prefix + newLoop + suffix, 'utf8');
